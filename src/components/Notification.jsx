...
import {Button, Pie} from "./index";
import {chatData} from "../data/dummy";
import {useStateContext} from "../contexts/ContextProvider";
import {Kanban} from "../pages";

const Notification = () => {
    const {currentColor, setThemeNotification} = useStateContext();
    return (
        <div className="nav-item absolute right-5 md:right-40 top-16 bg-white dark:bg-secondary-dark-bg rounded-3xl bg-white dark:bg-[#4264D] p-8 rounded-lg w-96">
            <div className="flex justify-between items-center">
                <div className="flex gap-9">
                    ...
                    <button type="button"
                            onClick={() => setThemeNotification(false)}
                            style={{color: 'rgb(153, 171, 180)', borderRadius: '50%'}}
                            className="text-2xl p-3  hover:drop-shadow-xl hover:bg-light-gray">
                        <MdOutlineCancel/>
                    </button>
                </div>
                <Button icon={<MdOutlineCancel/>} color="rgb(153, 171, 181)" bgHoverColor="light-gray" size="2xl" borderRadius="50%"/>
            </div>
